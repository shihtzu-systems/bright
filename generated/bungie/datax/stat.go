...
func LoadStat(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("Stat [%d]", len(destiny.Stat.Values()))
	if !redis.Exists(data.StatDefinitionName) {
		log.Infof("Loading %d Stat", len(destiny.Stat.Values()))
...
func GetStat(hash int, redis tower.Redis) (out data.StatDefinition) {
