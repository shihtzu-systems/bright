...
func LoadPlugSet(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("PlugSet [%d]", len(destiny.PlugSet.Values()))
	if !redis.Exists(data.PlugSetDefinitionName) {
		log.Infof("Loading %d PlugSet", len(destiny.PlugSet.Values()))
