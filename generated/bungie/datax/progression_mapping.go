...
func LoadProgressionMapping(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("ProgressionMapping [%d]", len(destiny.ProgressionMapping.Values()))
	if !redis.Exists(data.ProgressionMappingDefinitionName) {
		log.Infof("Loading %d ProgressionMapping", len(destiny.ProgressionMapping.Values()))
