...
func LoadUnlockEvent(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("UnlockEvent [%d]", len(destiny.UnlockEvent.Values()))
	if !redis.Exists(data.UnlockEventDefinitionName) {
		log.Infof("Loading %d UnlockEvent", len(destiny.UnlockEvent.Values()))
...
func GetUnlockEvent(hash string, redis tower.Redis) (out data.UnlockEventDefinition) {
