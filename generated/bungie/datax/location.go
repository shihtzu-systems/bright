...
func LoadLocation(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("Location [%d]", len(destiny.Location.Values()))
	if !redis.Exists(data.LocationDefinitionName) {
		log.Infof("Loading %d Location", len(destiny.Location.Values()))
...
func GetLocation(hash string, redis tower.Redis) (out data.LocationDefinition) {
