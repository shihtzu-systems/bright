...
func LoadUnlock(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("Unlock [%d]", len(destiny.Unlock.Values()))
	if !redis.Exists(data.UnlockDefinitionName) {
		log.Infof("Loading %d Unlock", len(destiny.Unlock.Values()))
...
func GetUnlock(hash string, redis tower.Redis) (out data.UnlockDefinition) {
