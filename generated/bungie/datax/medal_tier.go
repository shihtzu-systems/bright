...
func LoadMedalTier(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("MedalTier [%d]", len(destiny.MedalTier.Values()))
	if !redis.Exists(data.MedalTierDefinitionName) {
		log.Infof("Loading %d MedalTier", len(destiny.MedalTier.Values()))
