...
func LoadItemCategory(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("ItemCategory [%d]", len(destiny.ItemCategory.Values()))
	if !redis.Exists(data.ItemCategoryDefinitionName) {
		log.Infof("Loading %d ItemCategory", len(destiny.ItemCategory.Values()))
