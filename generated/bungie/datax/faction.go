...
func LoadFaction(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("Faction [%d]", len(destiny.Faction.Values()))
	if !redis.Exists(data.FactionDefinitionName) {
		log.Infof("Loading %d Faction", len(destiny.Faction.Values()))
