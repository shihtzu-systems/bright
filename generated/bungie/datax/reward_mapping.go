...
func LoadRewardMapping(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("RewardMapping [%d]", len(destiny.RewardMapping.Values()))
	if !redis.Exists(data.RewardMappingDefinitionName) {
		log.Infof("Loading %d RewardMapping", len(destiny.RewardMapping.Values()))
...
func GetRewardMapping(hash string, redis tower.Redis) (out data.RewardMappingDefinition) {
