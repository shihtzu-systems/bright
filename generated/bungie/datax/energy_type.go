...
func LoadEnergyType(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("EnergyType [%d]", len(destiny.EnergyType.Values()))
	if !redis.Exists(data.EnergyTypeDefinitionName) {
		log.Infof("Loading %d EnergyType", len(destiny.EnergyType.Values()))
