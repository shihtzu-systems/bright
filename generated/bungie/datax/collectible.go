...
func LoadCollectible(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("Collectible [%d]", len(destiny.Collectible.Values()))
	if !redis.Exists(data.CollectibleDefinitionName) {
		log.Infof("Loading %d Collectible", len(destiny.Collectible.Values()))
...
func GetCollectible(hash string, redis tower.Redis) (out data.CollectibleDefinition) {
