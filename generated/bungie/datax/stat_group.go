...
func LoadStatGroup(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("StatGroup [%d]", len(destiny.StatGroup.Values()))
	if !redis.Exists(data.StatGroupDefinitionName) {
		log.Infof("Loading %d StatGroup", len(destiny.StatGroup.Values()))
...
func GetStatGroup(hash string, redis tower.Redis) (out data.StatGroupDefinition) {
