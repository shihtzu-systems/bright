...
func LoadReportReasonCategory(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("ReportReasonCategory [%d]", len(destiny.ReportReasonCategory.Values()))
	if !redis.Exists(data.ReportReasonCategoryDefinitionName) {
		log.Infof("Loading %d ReportReasonCategory", len(destiny.ReportReasonCategory.Values()))
