...
func LoadRace(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("Race [%d]", len(destiny.Race.Values()))
	if !redis.Exists(data.RaceDefinitionName) {
		log.Infof("Loading %d Race", len(destiny.Race.Values()))
...
func GetRace(hash string, redis tower.Redis) (out data.RaceDefinition) {
