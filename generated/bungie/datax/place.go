...
func LoadPlace(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("Place [%d]", len(destiny.Place.Values()))
	if !redis.Exists(data.PlaceDefinitionName) {
		log.Infof("Loading %d Place", len(destiny.Place.Values()))
