...
func LoadEnemyRace(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("EnemyRace [%d]", len(destiny.EnemyRace.Values()))
	if !redis.Exists(data.EnemyRaceDefinitionName) {
		log.Infof("Loading %d EnemyRace", len(destiny.EnemyRace.Values()))
...
func GetEnemyRace(hash string, redis tower.Redis) (out data.EnemyRaceDefinition) {
