...
func LoadUnlockValue(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("UnlockValue [%d]", len(destiny.UnlockValue.Values()))
	if !redis.Exists(data.UnlockValueDefinitionName) {
		log.Infof("Loading %d UnlockValue", len(destiny.UnlockValue.Values()))
...
func GetUnlockValue(hash string, redis tower.Redis) (out data.UnlockValueDefinition) {
