...
func LoadRewardItemList(destiny *data.Content, redis tower.Redis, overwrite bool) {
	log.Infof("RewardItemList [%d]", len(destiny.RewardItemList.Values()))
	if !redis.Exists(data.RewardItemListDefinitionName) {
		log.Infof("Loading %d RewardItemList", len(destiny.RewardItemList.Values()))
...
func GetRewardItemList(hash string, redis tower.Redis) (out data.RewardItemListDefinition) {
